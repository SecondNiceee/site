    ...
    <>
        ...
        <div className="container mx-auto px-4 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            <div className="hidden lg:flex items-center gap-4">
              <button
                onClick={toggleTheme}
                className="p-2 rounded-full hover:bg-secondary transition-colors hover:scale-110 active:scale-90 duration-200"
      ...
      {menuMounted && (
        ...
        >
          ...
          />
          <nav
            className={`absolute top-0 right-0 h-full w-[80%] max-w-sm bg-background border-l border-border p-8 pt-24 transition-transform duration-400 ease-out ${
            ...
            <div className="flex flex-col gap-6">
              {visibleNavLinks.map((link, index) => (
                <button
                  key={link.href}
                  onClick={() => scrollToSection(link.href)}
                  className="text-lg font-medium text-left hover:text-[oklch(0.75_0.18_50)] transition-all duration-200"
                  style={{
                    transitionDelay: isMobileMenuOpen ? `${index * 0.06}s` : "0s",
                    opacity: isMobileMenuOpen ? 1 : 0,
                    transform: isMobileMenuOpen ? "translateX(0)" : "translateX(20px)",
                    transition: `opacity 0.3s ease ${index * 0.06}s, transform 0.3s ease ${index * 0.06}s, color 0.2s`,
                  }}
                >
                  {link.label}
                </button>
