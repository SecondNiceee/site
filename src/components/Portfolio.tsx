...
export default function Portfolio() {
  ...
  const gridRef = useRevealChildren<HTMLDivElement>(120);
  ...
  return (
    <section id="portfolio" className="py-24 md:py-32 relative overflow-hidden bg-card/30">
      ...
      <div className="absolute top-1/2 left-0 w-72 h-72 bg-[oklch(0.75_0.18_50)/5] rounded-full blur-2xl -translate-x-1/2 -translate-y-1/2 pointer-events-none" style={{ transform: 'translate(-50%, -50%) translateZ(0)' }} />
      <div className="absolute top-1/3 right-0 w-96 h-96 bg-[oklch(0.75_0.18_50)/3] rounded-full blur-2xl translate-x-1/2 pointer-events-none" style={{ transform: 'translateX(50%) translateZ(0)' }} />
        ...
        {!isLoading && items.length > 0 && (
          ...
          >
            {items.map((item) => (
              <div
                key={item.id}
                className="reveal fade-up group relative rounded-3xl overflow-hidden bg-card border border-border hover:border-[oklch(0.75_0.18_50)/30] transition-all duration-300"
              >
                {/* Image */}
